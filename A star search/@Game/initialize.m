...
%run myAstar.m
astar_arr=myAstar()
astar_arr=[astar_arr 0 0 0 3 3 3];
iteration=1;
...
%        player2 = game.PlayerList{2};
        yu=randi([1 5]);
        if iteration<=length(astar_arr)
            yu=astar_arr(iteration);
        else
            stop(a);
        end
        if yu==1
    ...
    function checkAlive
        ...
        for i = 1:length(game.PlayerList)
            if game.PlayerList{i}.IsAlive == 0
                stop(a)
                warndlg(['Player ' game.PlayerList{i}.Name ' has reached the goal'],'Mubarakan!')
            end
        end
    end
